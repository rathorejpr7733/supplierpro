...
export interface GoodsInElement {
  group: number;
  description: string;
  brand: number;
  parent: string;
...
const ELEMENT_DATA: GoodsInElement[] = [
  {
    group: 2001,
...
displayedColumns: string[] = [
  ...
  selection = new SelectionModel<GoodsInElement>(true, []);
  ...
  checkboxLabel(row?: GoodsInElement): string {
    if (!row) {
