...
export interface ReturnNotesElement  {
      id:string;
      customer:string;
      reference: number;
      creation:string
...
}const ELEMENT_DATA: ReturnNotesElement[] = [
  {
    id: 'SO-001',
...
@Component({
  ...
  styleUrl: './return-notes.component.scss'
})
export class ReturnNotesComponent {
   ...
   selection = new SelectionModel<ReturnNotesElement>(true, []);
...
checkboxLabel(row?: ReturnNotesElement): string {
  if (!row) {
