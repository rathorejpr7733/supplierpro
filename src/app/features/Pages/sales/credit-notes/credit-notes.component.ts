...
export interface CreditNotesElement  {
      id:string;
      customer:string;
      reference: number;
      creation:string
      ...
      shipping:string
      action:string;
}const ELEMENT_DATA: CreditNotesElement[] = [
  {
    id: 'SO-001',
...
export class CreditNotesComponent {
   ...
   selection = new SelectionModel<CreditNotesElement>(true, []);
...
checkboxLabel(row?: CreditNotesElement): string {
  if (!row) {
